...
const resources = {
  zh: { translation: {
    brand: { subtitle: '在线账号成活与额度检测', version: 'v0.8.3' },
    ...
    actions: { importFolder: '导入文件夹', importFiles: '导入文件', pasteJson: '粘贴 JSON', validate: '在线验证', pauseValidation: '暂停验证', resumeValidation: '继续验证', cancelValidation: '取消验证', cancellingValidation: '正在取消', deleteInvalid: '删除失效', exportRemaining: '导出剩余', clear: '清空', close: '关闭', cancel: '取消', refreshQuota: '刷新额度', previous: '上一步', next: '下一步', finish: '完成', exitGuide: '退出指引', retryIp: '重新检测 IP', startValidation: '开始验证', confirmDelete: '确认删除', parseImport: '解析并导入', understood: '知道了' },
    ...
    validation: { eyebrow: '实际请求', title: '在线验证账号', foreignIp: '必须开启国外 IP / 代理', foreignIpHint: '验证请求和 IP 检测都从本机原生网络发出；中国大陆出口或检测失败时不能开始。', ipConfirmed: '国外 IP 已确认', checkingIp: '正在检测出口 IP', networkBlocked: '当前网络不可验证', available: '{{count}} 个账号可验证', hint: '账号会按顺序逐个验证，同一时间只发送 1 个请求。OAuth 会读取 Codex 5 小时/周额度。', checkingAccount: '正在串行验证该账号', scope: '验证范围', batch: '本批 25 个', all: '全部待验证账号（串行）' },
    ...
    notices: { importComplete: '{{source}}完成：{{files}} 个文件，解析 {{accounts}} 个账号', issues: '，{{count}} 个问题', import: '导入', fileImport: '文件导入', folderImport: '文件夹“{{name}}”导入', noJson: '所选文件夹内没有 JSON 文件', folderFailed: '文件夹导入失败：{{message}}', folderFailedGeneric: '文件夹导入失败', jsonOnly: '仅支持 JSON 文件', tooLarge: '文件超过 10 MB', checkingIp: '正在检测当前出口 IP...', ipCheckFailed: '出口 IP 检测失败，已阻止在线验证', noAccounts: '没有需要验证的账号', validationServiceFailed: '本地验证服务连接失败', tokenRefreshed: '访问令牌已刷新', validationComplete: '在线验证完成：本批次 {{count}} 个凭据', validationCancelled: '验证已取消：完成 {{done}} / {{total}} 个凭据', quotaRefreshed: '额度已刷新', quotaUnavailable: '未获取到额度：{{detail}}', quotaFailed: '额度刷新失败，请检查国外代理连接', exported: '已导出 {{count}} 个剩余账号', deleted: '已删除 {{count}} 个凭据失效账号', notValidated: '未验证', drop: '释放以导入 JSON' },
  } },
  en: { translation: {
    brand: { subtitle: 'Account health and Codex quota checks', version: 'v0.8.3' },
    ...
    actions: { importFolder: 'Import folder', importFiles: 'Import files', pasteJson: 'Paste JSON', validate: 'Validate', pauseValidation: 'Pause validation', resumeValidation: 'Resume validation', cancelValidation: 'Cancel validation', cancellingValidation: 'Cancelling', deleteInvalid: 'Delete invalid', exportRemaining: 'Export remaining', clear: 'Clear', close: 'Close', cancel: 'Cancel', refreshQuota: 'Refresh quota', previous: 'Previous', next: 'Next', finish: 'Finish', exitGuide: 'Exit guide', retryIp: 'Check IP again', startValidation: 'Start validation', confirmDelete: 'Delete', parseImport: 'Parse and import', understood: 'Got it' },
    ...
    validation: { eyebrow: 'Real request', title: 'Validate accounts', foreignIp: 'A non-mainland-China IP / proxy is required', foreignIpHint: 'Validation and IP checks use the device native network. Mainland China exits and failed checks are blocked.', ipConfirmed: 'Foreign IP confirmed', checkingIp: 'Checking exit IP', networkBlocked: 'Network cannot validate', available: '{{count}} accounts can be validated', hint: 'Accounts are validated sequentially with only one request in flight. OAuth also reads the Codex 5-hour and weekly quota.', checkingAccount: 'Sequentially validating this account', scope: 'Validation scope', batch: 'Next 25', all: 'All pending accounts (sequential)' },
